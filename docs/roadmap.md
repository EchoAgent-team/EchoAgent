...
- LLM client: the project has standardized on `LLMClient(provider="groq", ...)` from `prompt_parser.py`. Structured agents request JSON mode when possible. Groq can reject JSON mode for `openai/gpt-oss-120b` with `json_validate_failed`, so `LLMClient.generate()` falls back to a plain chat completion on that specific error. `build_playlist_node()` also falls back to already-ranked candidates if the final `PlaylistBuilderAgent` cannot produce valid JSON.
